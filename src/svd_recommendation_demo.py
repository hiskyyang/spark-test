...
spark = (
    SparkSession.builder.appName("SVD_ECommerce_Recommendation")
...
# ✅ 2. 加载评分数据
ratings_df = spark.read.csv("ratings_large.csv", header=True, inferSchema=True)
...
# ✅ 4. 转换为 RDD[Rating]
ratings_rdd = ratings_df.select("userId", "productId", "rating").rdd.map(
    lambda row: Rating(int(row["userId"]), int(row["productId"]), float(row["rating"]))
)
...
flatRecs = (
    userRecs.flatMapValues(lambda recs: recs)
...
productInfo = spark.read.csv("products_large.csv", header=True, inferSchema=True)
